...
function App() {
  ...
  const login = async () => {
    try {
      ...
      setUserId(res.data.userId); // ✅ FIXED HERE
      setIsLoggedIn(true);
      setPage('animals');
    } catch (err) {
      console.log(err);
  ...
  const getTotalMilk = () =>
    animals.reduce((sum, a) => sum + a.milkPerDay, 0);
  ...
  const getTotalDeliveredMilk = () =>
    deliveries.reduce((sum, d) => sum + d.litres, 0);

  const getRemainingMilk = () =>
    getTotalMilk() - getTotalDeliveredMilk();
  ...
  return (
    <div className="container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <h2>🐄 Dairy App</h2>
        <div onClick={() => setPage('home')} style={{ fontSize: 32, cursor: 'pointer' }}>
          🏡
        </div>
      </div>
      ...
      {page === 'home' && (
        <div className="card">
          ...
          <p><b>Farm Address:</b> Pothavaram</p>
      ...
      <div className="navbar">
        <div onClick={() => setPage('animals')}>🐄 Animals</div>
        <div onClick={() => setPage('delivery')}>🚚 Delivery</div>
        <div onClick={() => setPage('customer')}>👤 Customer</div>
        <div onClick={() => setPage('billing')}>💰 Billing</div>
      </div>
